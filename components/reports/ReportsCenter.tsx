...
function ServiceHistoryReport({ dataset }: { dataset: ReportsDataset }) {
  ...
  const exportRows = rows.map((row) => [row.service.serviceDate, formatTime(row.service.serviceTime, ""), serviceName(row), row.service.notes, row.members, row.visitors, row.sundaySchoolKids, row.total]);
  return (
    <section className="report-section print-report">
...
function MemberReport({ dataset }: { dataset: ReportsDataset }) {
  ...
  return (
    <section className="report-section print-report">
      <header className="report-section-heading"><div><p className="eyebrow">Individual attendance</p><h2>Member attendance</h2><p>Search a member to review completed-service attendance.</p></div>{report && <PrintExportActions onExport={() => downloadCsv(`ALUPC-${report.person.displayName.replace(/\W+/g, "-")}-attendance.csv`, ["Date", "Service", "Type", "Time"], report.services.map((row) => [row.service.serviceDate, serviceName(row), row.service.serviceType, formatTime(row.service.serviceTime, "")]))} />}</header>
